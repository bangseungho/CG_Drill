...
public:
	...
	bool check_object(Object* other) {
		GLfloat distance = sqrt(pow((world_position.x - other->cur_loc.x), 2) + pow((world_position.z - other->cur_loc.z), 2));
		return distance <= 0.8;
	}

	int collide(Object* other) {
		if (other->type == Type::crush_wall || other->type == Type::wall)
			if (world_position.x + 0.05 > other->cur_loc.x - 0.27 &&
				world_position.x - 0.05 < other->cur_loc.x + 0.27 &&
				world_position.z + 0.05 > other->cur_loc.z - 0.27 &&
				world_position.z - 0.05 < other->cur_loc.z + 0.27)
			{
				if (world_position.x > other->cur_loc.x + 0.2 && world_position.z -0.05 < other->cur_loc.z + 0.2 && world_position.z + 0.05 > other->cur_loc.z - 0.2) return 1;
				if (world_position.x < other->cur_loc.x - 0.2 && world_position.z -0.05 < other->cur_loc.z + 0.2 && world_position.z + 0.05 > other->cur_loc.z - 0.2) return 2;
				if (world_position.z > other->cur_loc.z + 0.2 && world_position.x -0.05 < other->cur_loc.x + 0.2 && world_position.x + 0.05 > other->cur_loc.x - 0.2) return 3;
				if (world_position.z < other->cur_loc.z - 0.2 && world_position.x -0.05 < other->cur_loc.x + 0.2 && world_position.x + 0.05 > other->cur_loc.x - 0.2) return 4;
		}
			return 0;
	}
...
static bool isCamMove = false;
static bool islowHeight = false;
static bool create_player = false;
static bool cam_fps = false;
static bool made_mage = false;
static int isZMove = 0;
...
GLvoid display()
{
	...
	glViewport(win_width - win_width * 0.25, win_height - win_height * 0.32, win_width * 0.3, win_height * 0.3);
...
void MoveTimerFuction(int value) {
	if (cam_fps) {
		...
		case Keyboard_Event::FOWARD:
			...
			for (auto& n : cube) {
				for (auto& val : n) {
					if (player.check_object(&val))
						if (player.collide(&val)) {
							player.translate(vec3(-fps_camera.Front.x * 0.01, 0, -fps_camera.Front.z * 0.01));
						}
				}
			}
...
void TimerFunction(int value)
{
	...
	if (isZMove == 1) {
		camera->TranslatePos(vec3(0, 0, 0.1));
	}
	if (isZMove == 2) {
		camera->TranslatePos(vec3(0, 0, -0.1));
	}
