...
Camera quarter_camera(Person_View::QUARTER, 0.0f, 1.0f, 2.0f, 0.0f, 1.0f, 0.0f, -90, -20);
...
GLvoid display()
{
	...
	for (auto& k : krain) {
		k.setting();
		k.draw(obj1_s_program, *camera, projection);
	}
...
void Init()
{
	...
	krain.push_back(Object{ glm::vec4(0.0f), "body.obj", "Plane" });
	krain.push_back(Object{ glm::vec4(0.0f), "body.obj", "UnderBody" });
	krain.push_back(Object{ glm::vec4(0.0f), "body.obj", "UpBody" });
	krain.push_back(Object{ glm::vec4(0.0f), "arm.obj", "LArm" });
	krain.push_back(Object{ glm::vec4(0.0f), "arm.obj", "RArm" });
	krain[0].lscale(vec3(3.0, -0.0001, 3.0));

	krain[1].lscale(vec3(0.2, 0.05, 0.2));
	krain[1].translate(vec3(0.0, 0.05, 0.0));

	krain[2].lscale(vec3(0.12, 0.04, 0.12));
	krain[2].lscale(vec3(0.12, 0.04, 0.12));
	krain[2].translate(vec3(0.0, 0.1, 0.0));

	krain[3].lscale(vec3(0.035, 0.1, 0.035));
	krain[3].translate(vec3(-0.05, 0.25, 0.0));

	krain[4].lscale(vec3(0.035, 0.1, 0.035));
	krain[4].translate(vec3(0.05, 0.25, 0.0));

	light_box.scale(vec3(0.01));
	light_box.world_pivot.y += 0.5;
	light_box.world_pivot.x += 2;
}
