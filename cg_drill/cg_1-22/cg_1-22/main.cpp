...
Camera quarter_camera(Person_View::QUARTER, -2.0f, 2.0f, 5.0f, 0.0f, 1.0f, 0.0f, -90, -90);
...
GLvoid display()
{
	...
	orb.setting();
	orb.draw(obj1_s_program, *camera, projection);
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case 'n':
	case 'N':
		isHexi = isHexi ? false : true;
		isHexi ? draw_obj = &cube : draw_obj = &quad;
		break;
	case 'm':
	case 'M':
...
void TimerFunction(int value)
{
	...
	if (isFarOrigin == 1) {
		...
		orb.world_scale.x += -from_origin.x / 50 + -from_origin.z / 50;
		orb.world_scale.z += -from_origin.x / 50 + -from_origin.z / 50;
	}
...
void Init()
{
	...
	orb.scale(vec3(3, 0.005, 3));
	cube.scale(vec3(1.0));
	light_box.scale(vec3(0.2));
